...
const PubTest = ({ dispatch, pubTest }) => {
  ...
  const queryFormProps = {
    ...
    formSubmit: (dat) => {
      dispatch({
        type: 'pubTest/queryList',
        payload: { tableParam: { ...dat, currentPage: 1 } },
      });
    },
    addClick: () => {
      dispatch({
        type: 'pubTest/toggleModal',
        payload: { type: 'add' },
      });
    },
    deleteClick: () => {
...
function mapStateToProps({ pubTest }) {
  return { pubTest };
}

export default connect(mapStateToProps)(PubTest);
