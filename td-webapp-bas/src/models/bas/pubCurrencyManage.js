import * as service from '../../services/bas/pubCurrency';
import * as countryService from '../../services/bas/pubCountry';
import { parseResponse } from '../../utils/request';
import { callNotice } from '../../utils/alert';
import * as i18n from '../../utils/i18n';

// 基础配置信息
const namespace = 'pubCurrencyManage';
const objectId = 'currencyCode';
const enterPath = '/bas/basDataManage/pubCurrencyManage';
// 基础公共信息
...
export default {
    ...
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === enterPath) {
          ...
          dispatch({ type: 'queryCountryList', payload: { miniFormTableParam: { currentPage: 1 } } });
        }
      });
    },
  },
  effects: {
    ...
    * queryCountryList({ payload }, { call, put }) {
      ...
      console.log('payload.miniFormTableParam==', payload.miniFormTableParam)
      const res = yield call(countryService.queryList, { ...payload.miniFormTableParam });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateState',
