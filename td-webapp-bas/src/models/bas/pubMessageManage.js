import * as service from '../../services/bas/pubMessage';
import { parseResponse } from '../../utils/request';
import { callNotice } from '../../utils/alert';
import * as i18n from '../../utils/i18n';

// 基础配置信息
const namespace = 'pubMessageManage';
const objectId = 'MSG_ID';
const enterPath = '/bas/messageManage/pubMessageManage';
// 基础公共信息
...
export default {
  ...
  effects: {
    ...
    * resend({ payload }, { call, put, select }) {
      const res = yield call(service.resend, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        callNotice(commonMap.success, detail.rspMsg || commonMap.success, 'success');
        const tableParam = yield select(state => state[namespace].tableParam);
