import * as service from '../../services/agtp/agentRole';
...
// 基础配置信息
const namespace = 'agentRoleManage';
const objectId = 'roleId';
const enterPath = '/agtp/authManage/AgentRoleManage';
// 基础公共信息
...
export default {
  ...
  effects: {
    ...
    * updateOne({ payload }, { call, put, select }) {
      ...
      if (detail.rspCod === '200') {
        ...
        if (result.rspCod === '200') {
          yield put({
            type: 'updateState',
            payload: { tableList: result.rspList, tableTotal: result.total, tableCurrentPage: 1 },
          });
        }
        yield put(tableLoadFinOpt);
      } else {
    ...
    * queryRoleMenu({ payload }, { call, put }) {
      const res = yield call(service.queryRoleMenuInfo, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'toggleModal',
          payload: { type: 'assignMenu', data: detail.rspData },
        });
      } else {
        callNotice(commonMap.fail, detail.rspMsg || commonMap.failInfo, 'error');
      }
    },
    * assignRoleMenu({ payload }, { call, put }) {
      yield put({
        type: 'updateState',
        payload: { roleMenuFormSubmit: true },
      });
      const res = yield call(service.assignRoleMenu, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'assignRoleMenuSuccess',
          payload,
        });
        callNotice(commonMap.success, detail.rspMsg || commonMap.success, 'success');
      } else {
        yield put({
          type: 'updateState',
          payload: { roleMenuFormSubmit: false },
        });
        callNotice(commonMap.fail, detail.rspMsg || commonMap.failInfo, 'error');
      }
    },
  ...
  reducers: {
    ...
    toggleModal(state, action) {
      ...
      switch (type) {
        ...
        case 'info':
          ...
          break;
        case 'assignMenu':
          newState = { ...state, roleMenuFormData: action.payload.data, roleMenuModalVisible: !state.roleMenuModalVisible };
          break;
        default:
          break;
      }
    ...
    updateStatusSuccess(state, action) {
      ...
      const status = action.payload.isUse;
      const newTableList = state.tableList.map((item) => {
        if (ids.indexOf(item[objectId]) !== -1) {
          return { ...item, isUse: status };
        }
        return item;
      });
      return { ...state, tableLoading: false, tableList: newTableList };
    },
    assignRoleMenuSuccess(state, action) {
      const newItem = action.payload;
      const newTableList = state.tableList.map((item) => {
        if (item[objectId] === newItem[objectId]) {
          return { ...item, ...newItem };
      ...
      return { ...state, tableList: newTableList, roleMenuFormSubmit: false, roleMenuModalVisible: false };
    },
  },
};
