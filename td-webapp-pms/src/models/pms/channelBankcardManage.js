...
// 基础配置信息
const namespace = 'channelBankcardManage';
const objectId = 'chnId';
const enterPath = '/pms/channelManage/channelBankcardManage';
// 基础公共信息
...
export default {
  ...
  reducers: {
    ...
    toggleModal(state, action) {
      ...
      switch (type) {
        case 'add':
          ...
          break;
        case 'update':
          newState = { ...state, updateFormData: action.payload.data, updateModalVisible: !state.updateModalVisible, advExpand: false };
          break;
        case 'info':
    ...
    // flush component when update channel
    updateSuccess(state, action) {
      ...
      return { ...state, tableList: newTableList, updateFormSubmit: false, updateModalVisible: false, authUpdModalVisible: false, authUpdFormSubmit: false };
    },
    /* ====== 对于基本Manage页面 以上基本状态更新方法不需要修改 额外状态更新方法添加在下方 ====== */
    // flush component when enabled or disabled
    ...
    toggleAuthExpand(state, action) {
      ...
      newState = { ...state, authUpdModalVisible: !state.authUpdModalVisible, authUpdFormData: formData, chnId: chnId };
      return newState;
    },
  },
};
