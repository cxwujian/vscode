import * as service from '../../services/tms/terminalApp';
import * as miniFormService from '../../services/tms/terminalModel';
import { parseResponse } from '../../utils/request';
import { callNotice } from '../../utils/alert';
import * as i18n from '../../utils/i18n';

// 基础配置信息
const namespace = 'terminalAppManage';
const objectId = 'appId';
const enterPath = '/tms/terminalManage/terminalAppManage';
// 基础公共信息
...
export default {
    ...
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === enterPath) {
          ...
          dispatch({ type: 'queryTerModNoList', payload: { miniFormTableParam: { currentPage: 1 } } });
        }
      });
    },
  },
  effects: {
    ...
    * queryTerModNoList({ payload }, { call, put }) {
      yield put({
        type: 'updateState',
    ...
    toggleModal(state, action) {
      ...
      switch (type) {
        ...
        case 'update':
          ...
          newState = {
            ...state, updateFormData: formData, updateModalVisible: !state.updateModalVisible,
            filePathList: formData.filePathList,
            logoFilePath: formData.logoFilePath,
            appFilePath: formData.appFilePath,
          };
          break;
        case 'upload':
          formData = action.payload.data;
          newState = {
            ...state, uploadFormData: formData, uploadModalVisible: !state.uploadModalVisible,
            filePathList: formData.filePathList,
            logoFilePath: formData.logoFilePath,
            appFilePath: formData.appFilePath,
          };
