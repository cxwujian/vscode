import * as service from '../../services/bas/jobTriggerInfo';
import { parseResponse } from '../../utils/request';
import { callNotice } from '../../utils/alert';
import * as i18n from '../../utils/i18n';

// 基础配置信息
const namespace = 'jobTriggerInfoManage';
const objectId = 'id';
const enterPath = '/bas/jobTirgger/jobTriggerInfoManage';
// 基础公共信息
...
export default {
  ...
  effects: {
    ...
    * excute({ payload }, { call, put }) {
      yield put({
        type: 'updateState',
        payload: { updateFormSubmit: true },
      });
      const res = yield call(service.excute, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateSuccess',
    ...
    * pause({ payload }, { call, put }) {
      ...
      const res = yield call(service.pause, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateSuccess',
    ...
    * recovery({ payload }, { call, put }) {
      ...
      const res = yield call(service.recovery, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateSuccess',
