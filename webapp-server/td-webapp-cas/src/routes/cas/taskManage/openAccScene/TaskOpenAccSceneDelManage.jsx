...
const TaskOpenAccSceneDelManage = ({ dispatch, taskOpenAccSceneDelManage }) => {
  ...
  const cardProps = {
    title: bizMap.taskOpenAccSceneDelManage,
    style: { width: '100%' },
  };
  const todoQueryFormProps = {
    data: { modeltype: '1210', formReset },
    changeResetData: () => {
      dispatch({
        type: 'taskOpenAccSceneDelManage/updateState',
        payload: { formReset: false },
      });
    },
    formSubmit: (dat) => {
      const queryDat = dat;
      queryDat.modeltype = '1210';
      dispatch({
        type: 'taskOpenAccSceneDelManage/queryTodoList',
  ...
  const myApplyQueryFormProps = {
    ...
    formSubmit: (dat) => {
      ...
      queryDat.modeltype = '1210';
      dispatch({
        type: 'taskOpenAccSceneDelManage/queryMyApplyList',
        payload: { tableParam: { ...queryDat, currentPage: 1 } },
      });
    },
  };
  const myFinishQueryFormProps = {
    ...
    formSubmit: (dat) => {
      ...
      queryDat.modeltype = '1210';
      dispatch({
        type: 'taskOpenAccSceneDelManage/queryMyFinishList',
        payload: { tableParam: { ...queryDat, currentPage: 1 } },
      });
    },
  };
  const infoModalProps = {
    ...
    onCancel: () => {
      dispatch({
        type: 'taskOpenAccSceneDelManage/toggleModal',
  ...
  const updateFormProps = {
    ...
    formSubmitAgree: (dat) => {
      dispatch({
        type: 'taskOpenAccSceneDelManage/updateAgree',
        payload: { ...dat },
      });
    },
    formSubmitReject: (dat) => {
      dispatch({
        type: 'taskOpenAccSceneDelManage/updateReject',
        payload: { ...dat },
      });
    },
  };
  const todoFormProps = {
    ...
    handleAuditHisClick(record) {
      dispatch({
        type: 'taskOpenAccSceneDelManage/queryAuditHis',
        payload: { ...record },
      });
    },
    handleUpdateClick(record) {
      dispatch({
        type: 'taskOpenAccSceneDelManage/beforeUpdate',
        payload: { ...record },
      });
    },
  };
  const myApplyFormProps = {
    ...
    handleDetailClick(record) {
      dispatch({
        type: 'taskOpenAccSceneDelManage/queryOne',
        payload: { ...record },
      });
    },
    handleAuditHisClick(record) {
  ...
  const changeTabPane = (activeKey) => {
    ...
    if (activeKey === '1') {
      dispatch({
        type: 'taskOpenAccSceneDelManage/queryTodoList',
        payload: { tableParam: { modeltype: '1210', currentPage: 1 } },
      });
    } else if (activeKey === '2') {
      dispatch({
        type: 'taskOpenAccSceneDelManage/queryMyFinishList',
...
function mapStateToProps({ taskOpenAccSceneDelManage }) {
  return { taskOpenAccSceneDelManage };
}

export default connect(mapStateToProps)(TaskOpenAccSceneDelManage);
