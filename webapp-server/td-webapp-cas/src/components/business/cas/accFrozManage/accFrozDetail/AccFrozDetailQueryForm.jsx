...
const AccFrozDetailQueryForm = (props) => {
  ...
  const { form, formSubmit, advExpand, collapseClick,
  ...
  if (dat.frozAmtMin) {
    dat.frozAmtMin = standUnitToMinUnit(dat.frozAmtMin, dat.ccy);
  }
  if (dat.frozAmtMax) {
    dat.frozAmtMax = standUnitToMinUnit(dat.frozAmtMax, dat.ccy);
  }
  const param = encode(qs.stringify(dat), 'base64');
  const downloadUrl = `rest/cas/accFrozDetails/export?p=${param}`;
