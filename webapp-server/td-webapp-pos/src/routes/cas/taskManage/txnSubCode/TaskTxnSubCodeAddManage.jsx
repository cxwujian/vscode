...
const TaskTxnSubCodeAddManage = ({ dispatch, taskTxnSubCodeAddManage }) => {
  ...
  const cardProps = {
    title: bizMap.taskTxnSubCodeAddManage,
    style: { width: '100%' },
  };
  const todoQueryFormProps = {
    data: { modeltype: '1007', formReset },
    changeResetData: () => {
      dispatch({
        type: 'taskTxnSubCodeAddManage/updateState',
        payload: { formReset: false },
      });
    },
    formSubmit: (dat) => {
      const queryDat = dat;
      queryDat.modeltype = '1007';
      dispatch({
        type: 'taskTxnSubCodeAddManage/queryTodoList',
  ...
  const myApplyQueryFormProps = {
    ...
    formSubmit: (dat) => {
      ...
      queryDat.modeltype = '1007';
      dispatch({
        type: 'taskTxnSubCodeAddManage/queryMyApplyList',
        payload: { tableParam: { ...queryDat, currentPage: 1 } },
      });
    },
  };
  const myFinishQueryFormProps = {
    ...
    formSubmit: (dat) => {
      ...
      queryDat.modeltype = '1007';
      dispatch({
        type: 'taskTxnSubCodeAddManage/queryMyFinishList',
        payload: { tableParam: { ...queryDat, currentPage: 1 } },
      });
    },
  };
  const infoModalProps = {
    ...
    onCancel: () => {
      dispatch({
        type: 'taskTxnSubCodeAddManage/toggleModal',
  ...
  const updateFormProps = {
    ...
    formSubmitAgree: (dat) => {
      dispatch({
        type: 'taskTxnSubCodeAddManage/updateAgree',
        payload: { ...dat },
      });
    },
    formSubmitReject: (dat) => {
      dispatch({
        type: 'taskTxnSubCodeAddManage/updateReject',
        payload: { ...dat },
      });
    },
  };
  const todoFormProps = {
    ...
    handleAuditHisClick(record) {
      dispatch({
        type: 'taskTxnSubCodeAddManage/queryAuditHis',
        payload: { ...record },
      });
    },
    handleUpdateClick(record) {
      dispatch({
        type: 'taskTxnSubCodeAddManage/beforeUpdate',
        payload: { ...record },
      });
    },
  };
  const myApplyFormProps = {
    ...
    handleDetailClick(record) {
      dispatch({
        type: 'taskTxnSubCodeAddManage/queryOne',
        payload: { ...record },
      });
    },
    handleAuditHisClick(record) {
  ...
  const changeTabPane = (activeKey) => {
    ...
    if (activeKey === '1') {
      dispatch({
        type: 'taskTxnSubCodeAddManage/queryTodoList',
        payload: { tableParam: { modeltype: '1007', currentPage: 1 } },
      });
    } else if (activeKey === '2') {
      dispatch({
        type: 'taskTxnSubCodeAddManage/queryMyFinishList',
...
function mapStateToProps({ taskTxnSubCodeAddManage }) {
  return { taskTxnSubCodeAddManage };
}

export default connect(mapStateToProps)(TaskTxnSubCodeAddManage);
