...
const AccFrozDetailPageTable = (props) => {
  ...
  const tableProps = {
    ...
    tableColumns: [
      ...
      { title: bizMap.frozAmt, dataIndex: 'frozAmt', width: 100, render(text) { return cent2Yuan(text); } },
      { title: bizMap.thawAmt, dataIndex: 'thawAmt', width: 100, render(text) { return cent2Yuan(text); } },
      { title: bizMap.frozRsn, dataIndex: 'frozRsn', width: 100 },
      { title: bizMap.creTim, dataIndex: 'creDat', width: 100, render(text, record) { return record.creDat && record.creTim ? formatDateString(record.creDat + record.creTim) : ''; } },
      { title: bizMap.uptTim, dataIndex: 'uptDat', width: 100, render(text, record) { return record.uptDat && record.uptTim ? formatDateString(record.uptDat + record.uptTim) : ''; } },
