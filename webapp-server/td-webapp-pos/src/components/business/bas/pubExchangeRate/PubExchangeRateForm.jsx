...
const PubExchangeRateForm = (props) => {
  ...
  const handleSubmit = (ev) => {
    ...
    validateFieldsAndScroll((errors) => {
      if (errors) {
        ...
        if (data.toCurrency) {
          Object.assign(data, { toCurrencys: data.toCurrency.toString() });
        }
  ...
  if (data.toCurrencys) {
    currencySelected = data.toCurrencys.split(',');
  }
  const handleReset = () => {
    resetFields();
  }
    ...
    <Spin spinning={loading}>
      <Form layout="horizontal" onSubmit={handleSubmit}>
        <div hidden>
          <Row>
            <Col span={22}>
              <FormItem label={bizMap.exchangeId} {...formItemLayout} hasFeedback>
                {
                  getFieldDecorator('exchangeId', {
                    initialValue: data.exchangeId,
                    rules: [{
                      required: true,
                    }],
                  })(
                    <Input placeholder={bizMap.exchangeId} />,
                  )
                }
              </FormItem>
            </Col>
