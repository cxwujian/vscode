import * as service from '../../../../services/cas/task';
import { parseResponse } from '../../../../utils/request';
import { callNotice } from '../../../../utils/alert';
import * as i18n from '../../../../utils/i18n';

// 基础配置信息
const namespace = 'taskBusTypGroupAddManage';
const objectId = 'id';
const enterPath = '/cas/taskManage/taskBusTypGroupAddManage';
// 基础公共信息
...
export default {
  ...
  state: {
    tableCurrentPage: 1,
    advExpand: false,
    tableParam: { currentPage: 1, modeltype: '1001' },
    tableLoading: false,
    tableList: [],
    busIdTableList: [],
    tableTotal: 0,
    ...
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === enterPath) {
          dispatch({ type: 'queryTodoList', payload: { tableParam: { currentPage: 1, modeltype: '1001' } } });
        }
      });
    },
  },
  effects: {
    ...
    * beforeUpdate({ payload }, { call, put }) {
      ...
      if (detail.rspCod === '200') {
        yield put({
          type: 'toggleModal',
          payload: { type: 'updateBusIdList', data: detail.rspData, busIdTableList: detail.rspData.queryMap.busList, busIdTableTotal: detail.rspData.queryMap.total, tableCurrentPage: '1' },
        });
      } else {
        callNotice(commonMap.fail, detail.rspMsg || commonMap.failInfo, 'error');
      }
    ...
    * queryOne({ payload }, { call, put }) {
      ...
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateState',
          payload: { busIdTableList: detail.rspData.queryMap.busList, busIdTableTotal: detail.rspData.queryMap.total, tableCurrentPage: '1' },
        });
        yield put({
          type: 'toggleModal',
  ...
  reducers: {
    ...
    toggleModal(state, action) {
      ...
      switch (type) {
        case 'update':
          newState = {
            ...state, updateFormData: action.payload.data, tableList: action.payload.tableList, tableTotal: action.payload.tableTotal, tableCurrentPage: action.payload.tableCurrentPage,
        ...
        case 'updateBusIdList':
          newState = {
            ...state, updateFormData: action.payload.data, busIdTableList: action.payload.busIdTableList, busIdTableTotal: action.payload.busIdTableTotal, tableCurrentPage: action.payload.tableCurrentPage,
            updateModalVisible: !state.updateModalVisible,
          };
          break;
        default:
