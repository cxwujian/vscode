import * as service from '../../services/bas/pubExchangeRate';
import * as currencyService from '../../services/bas/pubCurrency';
import { parseResponse } from '../../utils/request';
import { callNotice } from '../../utils/alert';
import * as i18n from '../../utils/i18n';

// 基础配置信息
const namespace = 'pubExchangeRateManage';
const objectId = 'exchangeId';
const enterPath = '/bas/basDataManage/pubExchangeRateManage';
// 基础公共信息
...
export default {
  ...
  state: {
    ...
    infoTableData: {},
    currencyList: [],
    oneExchangeRatePageList: [],
    oneExchangeRateLoading: false,
    oneExchangeRateTotal: 0,
    oneExchangeRateCurrentPage: 1,
    oneExchangeRateParam: { currentPage: 1 },
  },
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === enterPath) {
          ...
          dispatch({ type: 'queryCurrencyList', payload: {} });
        }
      });
    },
  },
  effects: {
    ...
    * queryCurrencyList({ payload }, { call, put }) {
      const res = yield call(currencyService.queryAll);
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateState',
          payload: { currencyList: detail.rspList },
        });
      }
    },
    * queryOnePageList({ payload }, { call, put }) {
      const res = yield call(service.queryOnePageList, payload.record);
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateState',
          payload: { oneExchangeRatePageList: detail.rspList, infoModalVisible: true },
        });
      }
    },
    // enable or disable
  ...
  reducers: {
    ...
    updateStatusSuccess(state, action) {
      const exchangeIds = action.payload.exchangeIds;
      const status = action.payload.status;
      const newTableList = state.tableList.map((item) => {
        if (exchangeIds.indexOf(item[objectId]) !== -1) {
