...
// 基础配置信息
const namespace = 'terminalStockManage';
const objectId = 'terId';
const enterPath = '/tms/terminalInfoManage/terminalStockManage';
// 基础公共信息
...
export default {
  ...
  state: {
    ...
    stocksOutModalVisible: false,
    stockLogModalVisible: false,
    outType: '',
    stockLogList: [],
    terVerOptionsData: [],
    parModOptionsData: [],
    ...
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === enterPath) {
          ...
          // dispatch({ type: 'queryTerVerOptionData', payload: {} });
          // 查询终端模版下拉列表
          dispatch({ type: 'queryParModOptionData', payload: {} });
        }
      });
    },
  },
  effects: {
    ...
    * stocksOut({ payload }, { call, put, select }) {
      yield put(tableLoadOpt);
      const res = yield call(service.stocksOut, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'stocksOutSuccess',
          payload,
        });
        callNotice(commonMap.success, detail.rspMsg || commonMap.success, 'success');
        const tableParam = yield select(state => state[namespace].tableParam);
    ...
    * recoveryList({ payload }, { call, put, select }) {
      yield put(tableLoadOpt);
      const res = yield call(service.recoveryList, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'recoveryListSuccess',
          payload,
        });
        callNotice(commonMap.success, detail.rspMsg || commonMap.success, 'success');
        const tableParam = yield select(state => state[namespace].tableParam);
    ...
    * queryLogs({ payload }, { call, put }) {
      let stockRecord = {};
      stockRecord = payload.data;
      const res = yield call(service.queryLogs, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'toggleModal',
          payload: { type: 'updateLogList', logList: detail.rspList, stockRecord: stockRecord },
        });
      } else {
        callNotice(commonMap.fail, detail.rspMsg || commonMap.failInfo, 'error');
      }
    },
    * queryTerVerOptionData({ payload }, { call, put }) {
      const res = yield call(service.queryTerVerSelect, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateState',
          payload: { terVerOptionsData: detail.rspList },
        });
      } else {
        callNotice(commonMap.fail, detail.rspMsg || commonMap.failInfo, 'error');
      }
    },
    * queryTerVerOptionDataByCop({ payload }, { call, put }) {
      const res = yield call(service.queryTerVerSelectByCop, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateState',
    ...
    * queryParModOptionData({ payload }, { call, put }) {
      const res = yield call(service.queryTempSelect, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateState',
          payload: { allParModList: detail.rspList },
        });
      } else {
        callNotice(commonMap.fail, detail.rspMsg || commonMap.failInfo, 'error');
      }
  ...
  reducers: {
    ...
    toggleModal(state, action) {
      ...
      switch (type) {
        ...
        case 'info':
          ...
          break;
        case 'stocksOut': {
          const outType = action.payload.outType;
          const list = [];
          if (outType) {
            for (let i = 0; i < state.allParModList.length; i++) {
              const item = state.allParModList[i];
              if (item.type === outType) {  // 添加智能终端模版 or 传统终端模版
                list.push(item);
              }
            }
          }
          newState = { ...state, ids: action.payload.ids, parModOptionsData: list, stocksOutModalVisible: !state.stocksOutModalVisible };
          break;
        }
        case 'updateLogList':
          newState = { ...state, stockLogList: action.payload.logList, infoTableData: action.payload.stockRecord, stockLogModalVisible: !state.stockLogModalVisible };
          break;
        default:
          break;
      }
    ...
    stocksOutSuccess(state, action) {
      ...
      return { ...state, tableLoading: false, tableList: newTableList, updateFormSubmit: false, stocksOutModalVisible: false };
    },
    recoveryListSuccess(state, action) {
      const ids = action.payload.ids;
      const stoStatus = action.payload.stoStatus;
      const newTableList = state.tableList.map((item) => {
        if (ids.indexOf(item[objectId]) !== -1) {
          return { ...item, stoStatus: stoStatus };
        }
        return item;
      });
      return { ...state, tableLoading: false, tableList: newTableList };
    ...
    queryLogsSuccess(state, action) {
      ...
      return { ...state, tableList: newTableList, updateFormSubmit: false, stockLogModalVisible: false };
    },
  },
};
