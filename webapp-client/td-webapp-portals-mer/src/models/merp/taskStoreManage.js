import * as service from '../../services/merp/task';
...
// 基础配置信息
const namespace = 'taskStoreManage';
const objectId = 'id';
const enterPath = '/merp/store/taskStoreManage';
// 基础公共信息
...
export default {
  ...
  state: {
    tableCurrentPage: 1,
    advExpand: false,
    tableParam: { currentPage: 1, modeltype: '0030' },
    tableLoading: false,
    tableList: [],
    tableTotal: 0,
    tableSelects: [],
    ...
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === enterPath) {
          dispatch({ type: 'queryTodoList', payload: { tableParam: { currentPage: 1, modeltype: '0030' } } });
        }
      });
    },
  },
  effects: {
    * queryTodoList({ payload }, { call, put }) {
      ...
      // const res = yield call(service.queryTodoList, { ...payload.tableParam });
    ...
    * beforeUpdate({ payload }, { call, put }) {
      const res = yield call(service.queryTaskOne, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'toggleModal',
