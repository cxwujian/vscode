import * as service from '../../services/merp/merchantStoreUsr';
...
// 基础配置信息
const namespace = 'merchantStoreUsrManage';
const objectId = 'usrId';
const enterPath = '/merp/store/storeUsrManage';
// 基础公共信息
...
export default {
    ...
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === enterPath) {
          ...
          dispatch({ type: 'queryBras', payload: {} });
        }
      });
    },
  },
  effects: {
    ...
    * queryBras({ payload }, { call, put }) {
      const res = yield call(service.queryBras, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateState',
          payload: { braOptionsData: detail.rspList },
        });
      }
      yield put(tableLoadFinOpt);
    },
  ...
  reducers: {
    ...
    updateStatusSuccess(state, action) {
      ...
      const status = action.payload.usrStatus;
      const newTableList = state.tableList.map((item) => {
        if (ids.indexOf(item[objectId]) !== -1) {
          return { ...item, usrStatus: status };
        }
        return item;
      });
