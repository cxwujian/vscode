...
const ChnChkErrorPageTable = (props) => {
  const {
    tableList,
    tableTotal,
    tableCurrentPage,
  ...
  const tableProps = {
    rowKey: record => `${record.chnId}${record.chnBuzType}${record.chkDat}${record.txnId}${record.ttxnId}${record.tmerNo}${record.ttermNo}${record.seqNo}${record.batNo}`,
    tableColumns: [
      {
      ...
      {
        title: commonMap.action, fixed: 'right', width: 60, render(text, record) {
          return (
            <span>
              <a onClick={() => { handleDetailClick(record); }}>{bizMap.payDetail}</a>
              <span className="ant-divider" />
              <a onClick={() => { errorAccDealClick(record); }} disabled={record.dealSts === '02' || record.dealSts === '01'}>{bizMap.errorAccDeal}</a>
            </span>
          );
        },
      },
...
ChnChkErrorPageTable.propTypes = {
  ...
  errorAccDealClick: PropTypes.func,
};

ChnChkErrorPageTable.defaultProps = {
  tableList: [],
  tableTotal: 0,
  tableCurrentPage: 1,
  tableLoading: false,
  ...
  errorAccDealClick: noop,
};

export default ChnChkErrorPageTable;
