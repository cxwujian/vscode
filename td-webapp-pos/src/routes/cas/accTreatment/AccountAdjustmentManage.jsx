...
const AccountAdjustmentManage = ({ dispatch, accountAdjustmentManage }) => {
  ...
  const adjustmentAme = adjustmentAmt => (
    <p style={{ color: '#5C5C5C', fontSize: 16, fontWeight: '600' }}>{bizMap.adjustmentAmt}: <span style={{ color: '#ff4205', fontSize: 16 }} > { cent2Yuan(adjustmentAmt)}</span></p >
  )
  const adjustmentFormProps = {
    ...
    ccyOptionsData,
    formSubmit(dat) {
      callConfirm(bizMap.adjustmentConfirm, adjustmentAme(dat.adjustmentAmt), () => {
        dispatch({
