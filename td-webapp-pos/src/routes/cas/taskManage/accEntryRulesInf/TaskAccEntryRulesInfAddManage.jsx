...
const TaskAccEntryRulesInfAddManage = ({ dispatch, taskAccEntryRulesInfAddManage }) => {
  ...
  const cardProps = {
    title: bizMap.taskAccEntryRulesInfAddManage,
    style: { width: '100%' },
  };
  const todoQueryFormProps = {
    data: { modeltype: '1005', formReset },
    changeResetData: () => {
      dispatch({
        type: 'taskAccEntryRulesInfAddManage/updateState',
        payload: { formReset: false },
      });
    },
    formSubmit: (dat) => {
      const queryDat = dat;
      queryDat.modeltype = '1005';
      dispatch({
        type: 'taskAccEntryRulesInfAddManage/queryTodoList',
  ...
  const myApplyQueryFormProps = {
    ...
    formSubmit: (dat) => {
      ...
      queryDat.modeltype = '1005';
      dispatch({
        type: 'taskAccEntryRulesInfAddManage/queryMyApplyList',
        payload: { tableParam: { ...queryDat, currentPage: 1 } },
      });
    },
  };
  const myFinishQueryFormProps = {
    ...
    formSubmit: (dat) => {
      ...
      queryDat.modeltype = '1005';
      dispatch({
        type: 'taskAccEntryRulesInfAddManage/queryMyFinishList',
        payload: { tableParam: { ...queryDat, currentPage: 1 } },
      });
    },
  };
  const infoModalProps = {
    ...
    onCancel: () => {
      dispatch({
        type: 'taskAccEntryRulesInfAddManage/toggleModal',
  ...
  const updateFormProps = {
    ...
    formSubmitAgree: (dat) => {
      dispatch({
        type: 'taskAccEntryRulesInfAddManage/updateAgree',
        payload: { ...dat },
      });
    },
    formSubmitReject: (dat) => {
      dispatch({
        type: 'taskAccEntryRulesInfAddManage/updateReject',
        payload: { ...dat },
      });
    },
  };
  const todoFormProps = {
    ...
    handleAuditHisClick(record) {
      dispatch({
        type: 'taskAccEntryRulesInfAddManage/queryAuditHis',
        payload: { ...record },
      });
    },
    handleUpdateClick(record) {
      dispatch({
        type: 'taskAccEntryRulesInfAddManage/beforeUpdate',
        payload: { ...record },
      });
    },
  };
  const myApplyFormProps = {
    ...
    handleDetailClick(record) {
      dispatch({
        type: 'taskAccEntryRulesInfAddManage/queryOne',
        payload: { ...record },
      });
    },
    handleAuditHisClick(record) {
  ...
  const changeTabPane = (activeKey) => {
    ...
    if (activeKey === '1') {
      dispatch({
        type: 'taskAccEntryRulesInfAddManage/queryTodoList',
        payload: { tableParam: { modeltype: '1005', currentPage: 1 } },
      });
    } else if (activeKey === '2') {
      dispatch({
        type: 'taskAccEntryRulesInfAddManage/queryMyFinishList',
...
function mapStateToProps({ taskAccEntryRulesInfAddManage }) {
  return { taskAccEntryRulesInfAddManage };
}

export default connect(mapStateToProps)(TaskAccEntryRulesInfAddManage);
