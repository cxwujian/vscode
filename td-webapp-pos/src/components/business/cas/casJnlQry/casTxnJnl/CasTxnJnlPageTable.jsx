...
const CasTxnJnlPageTable = (props) => {
  ...
  const renderDataInfo = () => {
    return (
      <div>
        {bizMap.count} &nbsp;&nbsp;
        {`${bizMap.txnAmt}: ${cent2Yuan(amtCountInfo.sumTxnAmt)},`}&nbsp;&nbsp;
        {`${bizMap.feeAmt}: ${cent2Yuan(amtCountInfo.sumFeeAmt)}`}
      </div>
    );
  }
  const tableProps = {
    ...
    tableColumns: [
      ...
      { title: bizMap.feeAmt, dataIndex: 'feeAmt', width: 100, render(text) { return cent2Yuan(text); } },
