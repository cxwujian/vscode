...
const SubjectBalancePageTable = (props) => {
  ...
  const tableProps = {
    ...
    tableColumns: [
      {
        title: bizMap['subjectLev-1'], dataIndex: 'subjectLev1', width: 100, render(text, record) {
          let txt = '';
      ...
      {
        title: bizMap['subjectLev-2'], dataIndex: 'subjectLev2', width: 100, render(text, record) {
          let txt = '';
      ...
      {
        title: bizMap['subjectLev-3'], dataIndex: 'subjectLev3', width: 100, render(text, record) {
          let txt = '';
          ...
          if (record.subjectLev === '2' && record.children && record.children.length > 0) {
            if (record.accTyp === '1' || record.accTyp === '4') {
              for (let i = 0; i < record.children.length; i++) {
                txt += parseInt(record.children[i].debAccBal, 10);
              }
            } else if (record.accTyp === '2' || record.accTyp === '3') {
              for (let i = 0; i < record.children.length; i++) {
                txt += parseInt(record.children[i].creAccBal, 10);
              }
            }
            txt = cent2Yuan(txt);
          }
