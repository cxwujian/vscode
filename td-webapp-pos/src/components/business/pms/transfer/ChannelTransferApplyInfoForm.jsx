...
import { Form, Input, Button, Row, Col, Spin, Transfer, TimePicker, Select } from 'antd';
...
const ChannelTransferApplyInfoForm = (props) => {
  ...
  const { form, data, style, loading, submiting, formSubmit, handleTxnSup, bankTxnSup, optType, bankList, bankNos, bankNo, getChnInfo } = props;
  ...
  const handlerDatePickerChange = (value, dateString) => {
    setFieldsValue({ transferTimeStr: dateString });
  };

  const handlerDateEndPickerChange = (value, dateString) => {
    setFieldsValue({ transferTimeEnd: dateString });
  };
            ...
            <FormItem label={bizMap.chnAlias} {...formItemLayout} hasFeedback>
              {
                getFieldDecorator('chnAlias', {
                  ...
                  rules: [{ required: true, message: vaildMap.vaildChnChnAlias }],
                })(
                  <Input placeholder={bizMap.chnAlias} />,
                )
              }
            </FormItem>
          </Col>
