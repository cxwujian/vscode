...
export default {
  ...
  effects: {
    ...
    * queryBusIdSub({ payload }, { call, put }) {
      ...
      if (detail.rspCod === '200') {
        if (detail.rspList.length === 0) {
          yield put({
            type: 'toggleModal',
            payload: { ...payload, type: 'cancelBusId' },
          });
        } else {
          yield put({
            type: 'toggleModal',
            payload: { ...payload, type: 'updateSubList', subList: detail.rspList },
          });
        }
      } else {
        callNotice(commonMap.fail, detail.rspMsg || commonMap.failInfo, 'error');
    ...
    * disableOne({ payload }, { call, put, select }) {
      yield put(tableLoadOpt);
      const res = yield call(service.disableOne, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        callNotice(commonMap.success, detail.rspMsg || commonMap.success, 'success');
        // 获取当前命名控件的查询条件 重新查询列表
    ...
    toggleModal(state, action) {
      ...
      switch (type) {
        ...
        case 'info':
          ...
          break;
        case 'updateSubList':
          newState = { ...state, busIdSubList: action.payload.subList, busIdTableData: action.payload.data, subListModalVisible: !state.subListModalVisible };
          break;
        case 'cancelBusId':
          newState = { ...state, busIdTableData: action.payload.data, cancelModalVisible: !state.cancelModalVisible };
          break;
        case 'addBusId':
