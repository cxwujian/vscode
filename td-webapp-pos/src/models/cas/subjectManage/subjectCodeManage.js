import * as service from '../../../services/cas/subjectCode';
...
// 基础配置信息
const namespace = 'subjectCodeManage';
const objectId = 'subject';
const enterPath = '/cas/subjectManage/subjectCodeManage';
// 基础公共信息
...
export default {
  ...
  effects: {
    ...
    * queryBusIdList({ payload }, { call, put }) {
      yield put({
        type: 'updateState',
        payload: { ...payload, tableSelects: [], tableLoading: true, subBusIdModalVisible: true },
      });
      const tempBusData = {};
      tempBusData.tableLoading = true;
      yield put({
        type: 'updateState',
        payload: { busData: tempBusData },
      });
      const res = yield call(service.queryBusIdList, { ...payload.tableParam });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        tempBusData.tableList = detail.rspList;
        tempBusData.tableTotal = detail.total;
        tempBusData.tableCurrentPage = payload.tableParam.currentPage
        tempBusData.tableLoading = false;
        tempBusData.tableParam = payload.tableParam;
        tempBusData.expandedRowKeys = detail.rspData;
        yield put({
          type: 'updateState',
          payload: { busData: tempBusData },
        });
    ...
    toggleModal(state, action) {
      ...
      switch (type) {
        ...
        case 'info':
          ...
          break;
        case 'cancel':
          newState = { ...state, infoTableData: action.payload.data, cancelModalVisible: !state.cancelModalVisible };
          break;
        case 'updateBusIdList':
