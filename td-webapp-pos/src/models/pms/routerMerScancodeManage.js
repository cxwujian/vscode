...
export default {
  ...
  effects: {
    ...
    * queryOneList({ payload }, { call, put }) {
      ...
      yield put({
        type: 'updateState',
        payload: { ...payload, routerTableSelects: [], routerTableLoading: true, routerPayloadMerInfo: payload.data },
      });
      const res = yield call(service.queryOneList, { ...payload.data });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateState',
          payload: { routerTableList: detail.rspList, routerTableTotal: detail.total, routerTableLoading: false },
        });
      }
      yield put(tableLoadFinOpt);
    },
    ...
    * queryModList({ payload }, { call, put }) {
      if (payload.data.merId === null || payload.data.merId === undefined) {
        ...
        const res = yield call(service.queryModList, { ...payload });
        const detail = parseResponse(res);
        if (detail.rspCod === '200') {
          yield put({
            type: 'updateState',
    ...
    * setDefualt({ payload }, { call, put }) {
      ...
      const res = yield call(service.setDefualt, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateSuccess',
        ...
        if (result.rspCod === '200') {
          yield put({
            type: 'updateState',
            payload: { routerTableList: result.rspList, routerTableTotal: result.total },
          });
        }
      } else {
        yield put({
    ...
    * addOneMod({ payload }, { call, put, select }) {
      ...
      if (detail.rspCod === '200') {
        ...
        if (result.rspCod === '200') {
          yield put({
            type: 'updateState',
            payload: { routerTableList: result.rspList, routerTableTotal: result.total, routerTableLoading: false },
          });
        }
        yield put(routerTableLoadFinOpt);
      } else {
