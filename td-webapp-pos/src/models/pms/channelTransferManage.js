...
// 基础配置信息
const namespace = 'channelTransferManage';
const objectId = 'chnId';
const enterPath = '/pms/channelManage/channelTransferManage';
// 基础公共信息
  ...
  effects: {
    ...
    * setDefualt({ payload }, { call, put }) {
      ...
      if (detail.rspCod === '200') {
        ...
        yield put(tableLoadOpt);
        const param = {};
        const res = yield call(service.queryList, param);
        const result = parseResponse(res);
        if (result.rspCod === '200') {
          yield put({
            type: 'updateState',
     ...
     * updateBank({ payload }, { call, put }) {
      ...
      const res = yield call(service.updateBank, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateSuccess',
