...
const AccProfilesQueryForm = (props) => {
  ...
  const { form, formSubmit, addClick, enableClick, disableClick, advExpand, collapseClick,
    querySubjectList, subjectModalVisible, subjectData, onCancelSubjectModel, ccyOptionsData } = props;
  ...
  const handleSubmit = (ev) => {
    ...
    validateFields((errors) => {
      if (errors) {
        ...
        if (dat.accBalMin) {
          dat.accBalMin = standUnitToMinUnit(dat.accBalMin, dat.ccy);
        }
        if (dat.accBalMax) {
          dat.accBalMax = standUnitToMinUnit(dat.accBalMax, dat.ccy);
        }
        if (dat.avlAccBalMin) {
          dat.avlAccBalMin = standUnitToMinUnit(dat.avlAccBalMin, dat.ccy);
        }
        if (dat.avlAccBalMax) {
          dat.avlAccBalMax = standUnitToMinUnit(dat.avlAccBalMax, dat.ccy);
        }
        formSubmit(dat);
      }
    });
          ...
          <FormItem label={bizMap.ccy} {...formItemLayout}>
            {
              getFieldDecorator('ccy', {
                initialValue: commonMap.DEFAULT_CCY,
              })(
                <Select>
                  {
