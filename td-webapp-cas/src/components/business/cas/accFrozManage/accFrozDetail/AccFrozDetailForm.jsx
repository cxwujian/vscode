...
const AccFrozDetailForm = (props) => {
  ...
  let ableThawAmt = 0;
  if (data.thawAmt) {
    ableThawAmt = amtMinUnitToStandUnit((data.frozAmt - data.thawAmt), data.ccy);
  } else {
    ableThawAmt = amtMinUnitToStandUnit(data.frozAmt, data.ccy);
  }
  const handleSubmit = (ev) => {
    ...
    validateFieldsAndScroll((errors) => {
      if (errors) {
        ...
        if (Number(dat.thawAmt1) > Number(ableThawAmt)) {
          callNotice(commonMap.warning, bizMap.thawAmtNotice, 'warning');
          return;
        }
        if (dat.frozAmt) {
          dat.frozAmt = standUnitToMinUnit(dat.frozAmt, dat.ccy);
        }
        if (dat.thawAmt) {
          dat.thawAmt = standUnitToMinUnit(dat.thawAmt, dat.ccy);
        }
        dat.thawAmt1 = standUnitToMinUnit(dat.thawAmt1, dat.ccy);
        formSubmit(dat);
      }
    });
  };
  ...
  return (
    <Spin spinning={loading}>
      <Form layout="horizontal" onSubmit={handleSubmit}>
            ...
            <FormItem label={bizMap.frozAmt} {...formItemLayout} >
              {
                getFieldDecorator('frozAmt', {
                  initialValue: amtMinUnitToStandUnit(data.frozAmt, data.ccy),
                })(
            ...
            <FormItem label={bizMap.thawAmt} {...formItemLayout} >
              {
                getFieldDecorator('thawAmt', {
                  initialValue: amtMinUnitToStandUnit(data.thawAmt, data.ccy),
                })(
        ...
        <Row>
          ...
          <Col span={12}>
            <FormItem label={bizMap.creTim} {...formItemLayout} >
              {
                getFieldDecorator('creTim', {
                  initialValue: formatDateString(data.creTim),
                })(
        ...
        <Row>
          <Col span={12}>
            <FormItem label={bizMap.thawAmt1} {...formItemLayout} required >
              {
            ...
            <FormItem label={bizMap.thawRsn} {...formItemLayout} >
              {
                getFieldDecorator('thawRsn',
                  {
                    rules: [{ required: true, message: bizMap.validThawRsn }],
                  },
                )(
        ...
        <div hidden>
          <Row>
            <Col span={12}>
              <FormItem label={bizMap.busCde} {...formItemLayout} >
                {
                  getFieldDecorator('busCde', {
                    initialValue: data.busCde,
                  })(
                    <Input placeholder={bizMap.busCde} />,
                  )
                }
              </FormItem>
            </Col>
