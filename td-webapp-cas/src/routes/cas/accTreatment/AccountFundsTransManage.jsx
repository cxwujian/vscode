...
const AccountFundsTransManage = ({ dispatch, accountFundsTransManage }) => {
  ...
  const fundsTransAme = (fundsTransAmt, ccy) => (
    <p style={{ color: '#5C5C5C', fontSize: 16, fontWeight: '600' }}>{bizMap.fundsTransAmt}: <span style={{ color: '#ff4205', fontSize: 16 }} > {amtMinUnitToStandUnit(fundsTransAmt, ccy)}</span></p >
  )
  const fundsTransFormProps = {
    ...
    formSubmit(dat) {
      callConfirm(bizMap.FundsTransConfirm, fundsTransAme(dat.fundsTransAmt, dat.ccy), () => {
        dispatch({
