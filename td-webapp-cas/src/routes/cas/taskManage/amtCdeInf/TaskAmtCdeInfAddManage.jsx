...
const TaskAmtCdeInfAddManage = ({ dispatch, taskAmtCdeInfAddManage }) => {
  ...
  const cardProps = {
    title: bizMap.taskAmtCdeInfAddManage,
    style: { width: '100%' },
  };
  const todoQueryFormProps = {
    data: { modeltype: '1012', formReset },
    changeResetData: () => {
      dispatch({
        type: 'taskAmtCdeInfAddManage/updateState',
        payload: { formReset: false },
      });
    },
    formSubmit: (dat) => {
      const queryDat = dat;
      queryDat.modeltype = '1012';
      dispatch({
        type: 'taskAmtCdeInfAddManage/queryTodoList',
  ...
  const myApplyQueryFormProps = {
    ...
    formSubmit: (dat) => {
      ...
      queryDat.modeltype = '1012';
      dispatch({
        type: 'taskAmtCdeInfAddManage/queryMyApplyList',
        payload: { tableParam: { ...queryDat, currentPage: 1 } },
      });
    },
  };
  const myFinishQueryFormProps = {
    ...
    formSubmit: (dat) => {
      ...
      queryDat.modeltype = '1012';
      dispatch({
        type: 'taskAmtCdeInfAddManage/queryMyFinishList',
        payload: { tableParam: { ...queryDat, currentPage: 1 } },
      });
    },
  };
  const infoModalProps = {
    ...
    onCancel: () => {
      dispatch({
        type: 'taskAmtCdeInfAddManage/toggleModal',
  ...
  const updateFormProps = {
    ...
    formSubmitAgree: (dat) => {
      dispatch({
        type: 'taskAmtCdeInfAddManage/updateAgree',
        payload: { ...dat },
      });
    },
    formSubmitReject: (dat) => {
      dispatch({
        type: 'taskAmtCdeInfAddManage/updateReject',
        payload: { ...dat },
      });
    },
  };
  const todoFormProps = {
    ...
    handleAuditHisClick(record) {
      dispatch({
        type: 'taskAmtCdeInfAddManage/queryAuditHis',
        payload: { ...record },
      });
    },
    handleUpdateClick(record) {
      dispatch({
        type: 'taskAmtCdeInfAddManage/beforeUpdate',
        payload: { ...record },
      });
    },
  };
  const myApplyFormProps = {
    ...
    handleDetailClick(record) {
      dispatch({
        type: 'taskAmtCdeInfAddManage/queryOne',
        payload: { ...record },
      });
    },
    handleAuditHisClick(record) {
  ...
  const changeTabPane = (activeKey) => {
    ...
    if (activeKey === '1') {
      dispatch({
        type: 'taskAmtCdeInfAddManage/queryTodoList',
        payload: { tableParam: { modeltype: '1012', currentPage: 1 } },
      });
    } else if (activeKey === '2') {
      dispatch({
        type: 'taskAmtCdeInfAddManage/queryMyFinishList',
...
function mapStateToProps({ taskAmtCdeInfAddManage }) {
  return { taskAmtCdeInfAddManage };
}

export default connect(mapStateToProps)(TaskAmtCdeInfAddManage);
