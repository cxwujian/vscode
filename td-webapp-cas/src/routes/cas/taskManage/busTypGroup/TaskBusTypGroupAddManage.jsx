...
import BusTypGroupAudiPageTable from '../../../../components/business/cas/subjectManage/busTypGroup/BusTypGroupAudiPageTable';
import TaskAuditHisTable from '../../../../components/business/cas/task/TaskAuditHisTable';
import * as i18n from '../../../../utils/i18n';

const TabPane = Tabs.TabPane;
const TaskBusTypGroupAddManage = ({ dispatch, taskBusTypGroupAddManage }) => {
  ...
  const {
    ...
    auditHisModalVisible, auditHisData, busIdTableList, busIdTableTotal,
  } = taskBusTypGroupAddManage;
  const cardProps = {
    title: bizMap.taskBusTypGroupAddManage,
    style: { width: '100%' },
  };
  const todoQueryFormProps = {
    data: { modeltype: '1001', formReset },
    changeResetData: () => {
      dispatch({
        type: 'taskBusTypGroupAddManage/updateState',
        payload: { formReset: false },
      });
    },
    formSubmit: (dat) => {
      const queryDat = dat;
      queryDat.modeltype = '1001';
      dispatch({
        type: 'taskBusTypGroupAddManage/queryTodoList',
  ...
  const myApplyQueryFormProps = {
    ...
    formSubmit: (dat) => {
      ...
      queryDat.modeltype = '1001';
      dispatch({
        type: 'taskBusTypGroupAddManage/queryMyApplyList',
        payload: { tableParam: { ...queryDat, currentPage: 1 } },
      });
    },
  };
  const myFinishQueryFormProps = {
    ...
    formSubmit: (dat) => {
      ...
      queryDat.modeltype = '1001';
      dispatch({
        type: 'taskBusTypGroupAddManage/queryMyFinishList',
        payload: { tableParam: { ...queryDat, currentPage: 1 } },
      });
    },
  };
  const infoModalProps = {
    ...
    onCancel: () => {
      dispatch({
        type: 'taskBusTypGroupAddManage/toggleModal',
  ...
  const infoTableProps = {
    tableList: busIdTableList,
    tableTotal: busIdTableTotal,
  ...
  const updateModalProps = {
    ...
    onCancel: () => {
      dispatch({
        type: 'taskBusTypGroupAddManage/toggleModal',
        payload: { type: 'updateBusIdList', data: {} },
      });
    },
  };
  const updateFormProps = {
    data: updateFormData,
    busIdTableList: busIdTableList,
    busIdTableTotal: busIdTableTotal,
    tableCurrentPage: tableCurrentPage,
    submiting: updateFormSubmit,
    formSubmitAgree: (dat) => {
      dispatch({
        type: 'taskBusTypGroupAddManage/updateAgree',
        payload: { ...dat },
      });
    },
    formSubmitReject: (dat) => {
      dispatch({
        type: 'taskBusTypGroupAddManage/updateReject',
        payload: { ...dat },
      });
    },
  };
  const todoFormProps = {
    ...
    handleAuditHisClick(record) {
      dispatch({
        type: 'taskBusTypGroupAddManage/queryAuditHis',
        payload: { ...record },
      });
    },
    handleUpdateClick(record) {
      dispatch({
        type: 'taskBusTypGroupAddManage/beforeUpdate',
        payload: { ...record },
      });
    },
  };
  const myApplyFormProps = {
    ...
    handleDetailClick(record) {
      dispatch({
        type: 'taskBusTypGroupAddManage/queryOne',
        payload: { ...record },
      });
    },
    handleAuditHisClick(record) {
  ...
  const changeTabPane = (activeKey) => {
    ...
    if (activeKey === '1') {
      dispatch({
        type: 'taskBusTypGroupAddManage/queryTodoList',
        payload: { tableParam: { modeltype: '1001', currentPage: 1 } },
      });
    } else if (activeKey === '2') {
      dispatch({
        type: 'taskBusTypGroupAddManage/queryMyFinishList',
  ...
  return (
    <div>
      ...
      <Modal {...infoModalProps}>
        <BusTypGroupAudiPageTable {...infoTableProps} />
      </Modal>
...
function mapStateToProps({ taskBusTypGroupAddManage }) {
  return { taskBusTypGroupAddManage };
}

export default connect(mapStateToProps)(TaskBusTypGroupAddManage);
