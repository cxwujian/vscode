...
import TaskAuditForm from '../../../../components/business/cas/task/subAccCategory/TaskAuditForm';
import SubAccCategoryTable from '../../../../components/business/cas//accManage/subAccCategory/SubAccCategoryTable';
import TaskAuditHisTable from '../.././../../components/business/cas/task/TaskAuditHisTable';
import * as i18n from '../../../../utils/i18n';

const TabPane = Tabs.TabPane;
const TaskSubAccCategoryAddManage = ({ dispatch, taskSubAccCategoryAddManage }) => {
  ...
  const cardProps = {
    title: bizMap.taskSubAccCategoryAddManage,
    style: { width: '100%' },
  };
  const todoQueryFormProps = {
    data: { modeltype: '1003', formReset },
    changeResetData: () => {
      dispatch({
        type: 'taskSubAccCategoryAddManage/updateState',
        payload: { formReset: false },
      });
    },
    formSubmit: (dat) => {
      const queryDat = dat;
      queryDat.modeltype = '1003';
      dispatch({
        type: 'taskSubAccCategoryAddManage/queryTodoList',
  ...
  const myApplyQueryFormProps = {
    ...
    formSubmit: (dat) => {
      ...
      queryDat.modeltype = '1003';
      dispatch({
        type: 'taskSubAccCategoryAddManage/queryMyApplyList',
        payload: { tableParam: { ...queryDat, currentPage: 1 } },
      });
    },
  };
  const myFinishQueryFormProps = {
    ...
    formSubmit: (dat) => {
      ...
      queryDat.modeltype = '1003';
      dispatch({
        type: 'taskSubAccCategoryAddManage/queryMyFinishList',
        payload: { tableParam: { ...queryDat, currentPage: 1 } },
      });
    },
  };
  const infoModalProps = {
    ...
    onCancel: () => {
      dispatch({
        type: 'taskSubAccCategoryAddManage/toggleModal',
  ...
  const updateFormProps = {
    ...
    formSubmitAgree: (dat) => {
      dispatch({
        type: 'taskSubAccCategoryAddManage/updateAgree',
        payload: { ...dat },
      });
    },
    formSubmitReject: (dat) => {
      dispatch({
        type: 'taskSubAccCategoryAddManage/updateReject',
        payload: { ...dat },
      });
    },
  };
  const todoFormProps = {
    ...
    handleAuditHisClick(record) {
      dispatch({
        type: 'taskSubAccCategoryAddManage/queryAuditHis',
        payload: { ...record },
      });
    },
    handleUpdateClick(record) {
      dispatch({
        type: 'taskSubAccCategoryAddManage/beforeUpdate',
        payload: { ...record },
      });
    },
  };
  const myApplyFormProps = {
    ...
    handleDetailClick(record) {
      dispatch({
        type: 'taskSubAccCategoryAddManage/queryOne',
        payload: { ...record },
      });
    },
    handleAuditHisClick(record) {
  ...
  const changeTabPane = (activeKey) => {
    ...
    if (activeKey === '1') {
      dispatch({
        type: 'taskSubAccCategoryAddManage/queryTodoList',
        payload: { tableParam: { modeltype: '1003', currentPage: 1 } },
      });
    } else if (activeKey === '2') {
      dispatch({
        type: 'taskSubAccCategoryAddManage/queryMyFinishList',
  ...
  return (
    <div>
      ...
      <Modal {...infoModalProps}>
        <SubAccCategoryTable {...infoTableProps} />
      </Modal>
...
function mapStateToProps({ taskSubAccCategoryAddManage }) {
  return { taskSubAccCategoryAddManage };
}

export default connect(mapStateToProps)(TaskSubAccCategoryAddManage);
