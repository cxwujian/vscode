...
const TaskSubAccCategoryDelManage = ({ dispatch, taskSubAccCategoryDelManage }) => {
  ...
  const cardProps = {
    title: bizMap.taskSubAccCategoryDelManage,
    style: { width: '100%' },
  };
  const todoQueryFormProps = {
    data: { modeltype: '1103', formReset },
    changeResetData: () => {
      dispatch({
        type: 'taskSubAccCategoryDelManage/updateState',
        payload: { formReset: false },
      });
    },
    formSubmit: (dat) => {
      const queryDat = dat;
      queryDat.modeltype = '1103';
      dispatch({
        type: 'taskSubAccCategoryDelManage/queryTodoList',
  ...
  const myApplyQueryFormProps = {
    ...
    formSubmit: (dat) => {
      ...
      queryDat.modeltype = '1103';
      dispatch({
        type: 'taskSubAccCategoryDelManage/queryMyApplyList',
        payload: { tableParam: { ...queryDat, currentPage: 1 } },
      });
    },
  };
  const myFinishQueryFormProps = {
    ...
    formSubmit: (dat) => {
      ...
      queryDat.modeltype = '1103';
      dispatch({
        type: 'taskSubAccCategoryDelManage/queryMyFinishList',
        payload: { tableParam: { ...queryDat, currentPage: 1 } },
      });
    },
  };
  const infoModalProps = {
    ...
    onCancel: () => {
      dispatch({
        type: 'taskSubAccCategoryDelManage/toggleModal',
  ...
  const updateFormProps = {
    ...
    formSubmitAgree: (dat) => {
      dispatch({
        type: 'taskSubAccCategoryDelManage/updateAgree',
        payload: { ...dat },
      });
    },
    formSubmitReject: (dat) => {
      dispatch({
        type: 'taskSubAccCategoryDelManage/updateReject',
        payload: { ...dat },
      });
    },
  };
  const todoFormProps = {
    ...
    handleAuditHisClick(record) {
      dispatch({
        type: 'taskSubAccCategoryDelManage/queryAuditHis',
        payload: { ...record },
      });
    },
    handleUpdateClick(record) {
      dispatch({
        type: 'taskSubAccCategoryDelManage/beforeUpdate',
        payload: { ...record },
      });
    },
  };
  const myApplyFormProps = {
    ...
    handleDetailClick(record) {
      dispatch({
        type: 'taskSubAccCategoryDelManage/queryOne',
        payload: { ...record },
      });
    },
    handleAuditHisClick(record) {
  ...
  const changeTabPane = (activeKey) => {
    ...
    if (activeKey === '1') {
      dispatch({
        type: 'taskSubAccCategoryDelManage/queryTodoList',
        payload: { tableParam: { modeltype: '1103', currentPage: 1 } },
      });
    } else if (activeKey === '2') {
      dispatch({
        type: 'taskSubAccCategoryDelManage/queryMyFinishList',
...
function mapStateToProps({ taskSubAccCategoryDelManage }) {
  return { taskSubAccCategoryDelManage };
}

export default connect(mapStateToProps)(TaskSubAccCategoryDelManage);
