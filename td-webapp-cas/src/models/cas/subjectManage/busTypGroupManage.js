import * as service from '../../../services/cas/busTypGroup';
import { parseResponse } from '../../../utils/request';
import { callNotice } from '../../../utils/alert';
import * as i18n from '../../../utils/i18n';

// 基础配置信息
const namespace = 'busTypGroupManage';
const objectId = 'groupId';
const enterPath = '/cas/subjectManage/busTypGroupManage';
// 基础公共信息
...
export default {
  ...
  effects: {
    ...
    * queryBusIdSub({ payload }, { call, put }) {
      const res = yield call(service.queryBusIdSubList, { ...payload.data });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'toggleModal',
          payload: { ...payload, type: 'cancelBusId', subList: detail.rspList },
        });
      } else {
        callNotice(commonMap.fail, detail.rspMsg || commonMap.failInfo, 'error');
      }
    },
    * disableOne({ payload }, { call, put }) {
      const res = yield call(service.disableOne, { ...payload.record });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        callNotice(commonMap.success, detail.rspMsg || commonMap.success, 'success');
        // 获取当前命名控件的查询条件 重新查询列表
        const res = yield call(service.queryBusIdSubList, { ...payload.busIdTableData });
        const detail1 = parseResponse(res);
        if (detail1.rspCod === '200') {
          yield put({
            type: 'updateState',
            payload: { busIdSubList: detail1.rspList },
          });
        } else {
    ...
    * cancelBusId({ payload }, { call, put, select }) {
      const res = yield call(service.cancelBusId, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateSuccess',
        ...
        if (result.rspCod === '200') {
          yield put({
            type: 'updateState',
            payload: { tableList: result.rspList, tableTotal: result.total, cancelModalVisible: false },
          });
        }
        yield put(tableLoadFinOpt);
      } else {
    ...
    * enableOne({ payload }, { call, put, select }) {
      const res = yield call(service.enableOne, { ...payload });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        callNotice(commonMap.success, detail.rspMsg || commonMap.success, 'success');
        const tableParam = yield select(state => state[namespace].tableParam);
    ...
    * updateBusIdOne({ payload }, { call, put, select }) {
      yield put({
        type: 'updateState',
        payload: { updateFormSubmit: true },
      });
  ...
  reducers: {
    ...
    toggleModal(state, action) {
      ...
      switch (type) {
        case 'add':
          newState = { ...state, keys: [0], isReset: '1', addModalVisible: !state.addModalVisible, busTypData: action.payload.data };
          break;
        case 'update':
        ...
        case 'info':
          ...
          break;
        case 'cancelBusId':
          newState = { ...state, busIdSubList: action.payload.subList, busIdTableData: action.payload.data, cancelModalVisible: !state.cancelModalVisible };
          break;
        case 'addBusId':
          newState = { ...state, addModalVisible: !state.addModalVisible, busTypData: action.payload.data };
          break;
        case 'updateBusId':
          newState = { ...state, updateFormData: action.payload.data, updateBusIdModalVisible: !state.updateBusIdModalVisible };
          break;
        default:
          break;
      }
    ...
    updateBusIdSuccess(state, action) {
      const newItem = action.payload;
      const newTableList = state.tableList.map((item) => {
        if (item[objectId] === newItem[objectId]) {
          return { ...item, ...newItem };
      ...
      return { ...state, tableList: newTableList, updateFormSubmit: false, updateBusIdModalVisible: false };
    },
