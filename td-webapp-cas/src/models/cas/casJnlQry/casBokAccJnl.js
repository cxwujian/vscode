import * as service from '../../../services/cas/casBokAccJnl';
import * as subjectService from '../../../services/cas/subjectCode';
import * as transBaseService from '../../../services/cas/transBase';
...
// 基础配置信息
const namespace = 'casBokAccJnl';
const objectId = 'txnLog';
const enterPath = '/cas/casJnlQry/casBokAccJnl';
// 基础公共信息
...
export default {
    ...
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === enterPath) {
          ...
          dispatch({ type: 'queryTransBaseOptionsData', payload: { tableParam: { currentPage: 1 } } });
  ...
  effects: {
    ...
    * queryTransBaseOptionsData({ payload }, { call, put }) {
      const res = yield call(transBaseService.queryAllList, { ...payload.tableParam });
      const detail = parseResponse(res);
      if (detail.rspCod === '200') {
        yield put({
          type: 'updateState',
          payload: { transBaseOptionsData: detail.rspList },
        });
      } else {
        callNotice(commonMap.fail, detail.rspMsg || commonMap.failInfo, 'error');
      }
  ...
  reducers: {
    ...
    updateStatusSuccess(state, action) {
      ...
      const menuStatus = action.payload.menuStatus;
      const newTableList = state.tableList.map((item) => {
        if (ids.indexOf(item[objectId]) !== -1) {
          return { ...item, menuStatus: menuStatus };
        }
        return item;
      });
